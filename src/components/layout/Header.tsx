  ...
  return (
    <header className="absolute inset-x-0 top-0 z-50 px-[var(--page-pad)] pt-[27px]">
      <div className="mx-auto flex h-10 max-w-[1712px] items-center justify-between">
        ...
        <ButtonLink href="#appointment" className="hidden w-[168px] xl:inline-flex">Book an appointment</ButtonLink>
      ...
      <AnimatePresence>
        {open && <motion.nav initial={reduced ? false : { opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="mt-4 grid gap-1 border border-black/5 bg-white p-3 shadow-xl xl:hidden" aria-label="Mobile navigation">{navigation.map(([label, href]) => <Link key={label} href={href} onClick={() => setOpen(false)} className="px-4 py-3 text-sm font-bold uppercase tracking-wider hover:bg-ice">{label}</Link>)}<ButtonLink href="#appointment" className="mt-2">Book an appointment</ButtonLink></motion.nav>}
      </AnimatePresence>
    </header>
  );
}
