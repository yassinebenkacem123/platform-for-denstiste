  ...
  return (
    <form onSubmit={submit} noValidate className="grid gap-x-5 gap-y-5 sm:grid-cols-2" aria-describedby="appointment-form-status">
      ...
      <FormField label="Numéro de téléphone" htmlFor="phone" error={errors.phone}>
        <input id="phone" name="phone" type="tel" inputMode="tel" autoComplete="tel" required maxLength={25} placeholder="01 40 00 12 34" value={values.phone} onChange={event => update("phone", event.target.value)} onBlur={() => validateField("phone")} aria-invalid={Boolean(errors.phone)} aria-describedby={errors.phone ? "phone-error" : undefined} disabled={state === "loading"} className={inputClass} />
      </FormField>
