...
export function AppointmentForm() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setState("error");
      setStatusMessage("We could not send your request. Please check your connection and try again.");
    }
  }
  ...
  return (
    <form onSubmit={submit} noValidate className="grid gap-x-5 gap-y-5 sm:grid-cols-2" aria-describedby="appointment-form-status">
      <FormField label="Full name" htmlFor="fullName" error={errors.fullName}>
        <input id="fullName" name="fullName" autoComplete="name" required maxLength={100} placeholder="Your full name" value={values.fullName} onChange={event => update("fullName", event.target.value)} onBlur={() => validateField("fullName")} aria-invalid={Boolean(errors.fullName)} aria-describedby={errors.fullName ? "fullName-error" : undefined} disabled={state === "loading"} className={inputClass} />
      </FormField>
      <FormField label="Email address" htmlFor="email" error={errors.email}>
        <input id="email" name="email" type="email" autoComplete="email" required maxLength={254} placeholder="you@example.com" value={values.email} onChange={event => update("email", event.target.value)} onBlur={() => validateField("email")} aria-invalid={Boolean(errors.email)} aria-describedby={errors.email ? "email-error" : undefined} disabled={state === "loading"} className={inputClass} />
      </FormField>
      <FormField label="Phone number" htmlFor="phone" error={errors.phone}>
        <input id="phone" name="phone" type="tel" inputMode="tel" autoComplete="tel" required maxLength={25} placeholder="(212) 000-2299" value={values.phone} onChange={event => update("phone", event.target.value)} onBlur={() => validateField("phone")} aria-invalid={Boolean(errors.phone)} aria-describedby={errors.phone ? "phone-error" : undefined} disabled={state === "loading"} className={inputClass} />
      </FormField>
      <FormField label="Preferred appointment date" htmlFor="appointmentDate" error={errors.appointmentDate}>
      ...
      <FormField label="Preferred time" htmlFor="preferredTime" error={errors.preferredTime}>
      ...
      <FormField label="Service of interest" htmlFor="service" error={errors.service}>
      ...
      <FormField label="Message or additional notes" htmlFor="message" error={errors.message} className="sm:col-span-2">
        <textarea id="message" name="message" rows={4} maxLength={1000} placeholder="Tell us anything that would help us prepare for your visit." value={values.message} onChange={event => update("message", event.target.value)} onBlur={() => validateField("message")} aria-invalid={Boolean(errors.message)} aria-describedby={errors.message ? "message-error" : "message-help"} disabled={state === "loading"} className={`${inputClass} resize-y py-3`} />
        <p id="message-help" className="mt-1.5 text-xs tracking-wide text-[#697477]">Optional · up to 1,000 characters</p>
      </FormField>
...
function SubmitButton({ loading }: { loading: boolean }) {
  return (
    <button type="submit" disabled={loading} className="inline-flex min-h-12 min-w-[220px] items-center justify-center gap-3 bg-brand px-7 text-[11px] font-bold uppercase tracking-[.1em] text-white transition hover:bg-[#478da5] active:scale-[.98] disabled:cursor-wait disabled:bg-[#7fb5c7]">
      ...
      {loading ? "Sending request" : "Request an appointment"}
    </button>
  );
}
